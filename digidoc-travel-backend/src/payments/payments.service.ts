...
export interface PaginatedPaymentPlans {
  data: PaymentPlan[];
  total: number;
  page: number;
  limit: number;
...
export class PaymentsService {
  ...
  async createPlan(dto: CreatePlanDto, userId: string): Promise<PaymentPlan> {
    const plan = this.planRepo.create({
      ...dto,
      createdBy: userId,
    } as any) as unknown as PaymentPlan;
    const saved = (await this.planRepo.save(
      plan as any,
    )) as unknown as PaymentPlan;
    const amountPer = parseFloat(
    ...
    if (diff !== 0 && installments.length > 0)
      installments[installments.length - 1].amount = parseFloat(
        (amountPer + diff).toFixed(2),
      );
    await this.instRepo.save(installments);
    return this.findOne(saved.id);
  }

  async findAll(query: PaymentsQuery): Promise<PaginatedPaymentPlans> {
    ...
    const qb = this.planRepo
      ...
      .leftJoinAndSelect('plan.installmentsList', 'inst');
    if (query.studentId)
      qb.andWhere('plan.studentId = :sid', { sid: query.studentId });
    if (query.status)
  ...
  async findOne(id: string): Promise<PaymentPlan> {
    const plan = await this.planRepo.findOne({
      where: { id },
      relations: { student: true, installmentsList: { payments: true } },
    });
    if (!plan) throw new NotFoundException('Payment plan not found');
    return plan;
  }

  async getInstallments(planId: string): Promise<Installment[]> {
  ...
  async registerPayment(
    ...
    const saved = (await this.payRepo.save(
      payment as any,
    ...
    if (all.every((i) => i.status === 'paid')) {
      await this.planRepo.update(inst.planId, { status: 'completed' });
    }
    return saved;
  }

  async findPending(): Promise<Installment[]> {
    const now = new Date();
    const in7days = new Date();
    in7days.setDate(now.getDate() + 30);
    const pending = await this.instRepo
      .createQueryBuilder('inst')
      .leftJoinAndSelect('inst.plan', 'plan')
      .leftJoinAndSelect('plan.student', 'student')
      .where('inst.status = :status', { status: 'pending' })
      .andWhere('inst.dueDate <= :threshold', {
        threshold: in7days.toISOString().split('T')[0],
      })
