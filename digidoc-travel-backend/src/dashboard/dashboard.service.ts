...
export class DashboardService {
  ...
  async getSummary() {
    ...
    const expiringVisas = await this.visaRepo
      .createQueryBuilder('v')
      .where('v.expiryDate <= :threshold', {
        threshold: threshold.toISOString().split('T')[0],
      })
      .andWhere('v.expiryDate >= :now', {
        now: new Date().toISOString().split('T')[0],
      })
      .getCount();
    const expiredVisas = await this.visaRepo
      .createQueryBuilder('v')
    ...
    const pendingPayments = await this.instRepo.count({
      where: { status: 'pending' },
    });
    const overdue = await this.instRepo
      ...
      .getCount();
    const totalAmountRaw = await this.instRepo
      .createQueryBuilder('i')
    ...
    const nextEvents = await this.eventRepo
      .createQueryBuilder('e')
      .where('e.eventDate >= :now', { now: new Date() })
      .andWhere('e.eventDate <= :limit', { limit: in7days })
      .getCount();
    const totalEvents = await this.eventRepo.count();

    const startOfMonth = new Date();
    startOfMonth.setDate(1);
    const newThisMonth = await this.studentRepo
      .createQueryBuilder('s')
      .where('s.createdAt >= :start', { start: startOfMonth })
      .getCount();
    ...
    return {
      ...
      payments: {
        ...
        overdue,
        totalAmount: parseFloat(totalAmountRaw?.sum || '0'),
      },
  ...
  async getStudentsStats() {
    ...
    const byCountry = await this.studentRepo
      .createQueryBuilder('s')
      .select('s.countryOrigin', 'country')
      .addSelect('COUNT(*)', 'count')
      .groupBy('s.countryOrigin')
      .getRawMany();
  ...
  async getExpiringVisas() {
    const threshold = new Date();
    threshold.setDate(threshold.getDate() + 90);
    return this.visaRepo
      .createQueryBuilder('v')
      .leftJoinAndSelect('v.student', 's')
      .where('v.expiryDate <= :t', { t: threshold.toISOString().split('T')[0] })
      .andWhere('v.expiryDate >= :now', {
        now: new Date().toISOString().split('T')[0],
      })
      .getMany();
