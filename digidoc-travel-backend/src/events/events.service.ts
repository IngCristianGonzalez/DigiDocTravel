...
export class EventsService {
  constructor(
    ...
    private readonly partRepo: Repository<EventParticipant>,
  ) {}
  ...
  async create(dto: CreateEventDto, userId: string): Promise<Event> {
    ...
    if (dto.participantIds && dto.participantIds.length > 0) {
      const participants = dto.participantIds.map((sid) =>
        this.partRepo.create({ eventId: saved.id, studentId: sid } as any),
      );
  ...
  async findAll(query: EventsQuery): Promise<PaginatedEvents> {
    ...
    if (query.search)
      qb.andWhere('(event.title ILIKE :s OR event.description ILIKE :s)', {
        s: `%${query.search}%`,
      });
    if (query.upcoming === 'true')
      qb.andWhere('event.eventDate >= :now', { now: new Date() });
  ...
  async findOne(id: string): Promise<Event> {
    const event = await this.eventRepo.findOne({
      where: { id },
      relations: { participants: { student: true } },
    });
