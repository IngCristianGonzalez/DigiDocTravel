...
export interface VisasPaginationQuery {
  page?: string | number;
  limit?: string | number;
  studentId?: string;
  status?: string;
...
export interface VisaWithComputedStatus extends Visa {
...
export class VisasService {
  ...
  async create(dto: CreateVisaDto, userId: string): Promise<Visa> {
    if (new Date(dto.expiryDate) <= new Date(dto.issueDate))
    ...
    const result = saved as VisaWithComputedStatus;
    result.computedStatus = this.computeStatus(saved);
    result.daysLeft = Math.ceil(
      (new Date(saved.expiryDate).getTime() - Date.now()) /
        (1000 * 60 * 60 * 24),
    );
    return result;
  }

  async findAll(query: VisasPaginationQuery) {
    const page = parseInt(String(query.page)) || 1;
    const limit = parseInt(String(query.limit)) || 10;
    const qb = this.visaRepo
      .createQueryBuilder('visa')
      .leftJoinAndSelect('visa.student', 'student');
    if (query.studentId)
      qb.andWhere('visa.studentId = :sid', { sid: query.studentId });
    if (query.status)
      qb.andWhere('visa.status = :status', { status: query.status });
    if (query.search)
      qb.andWhere('(visa.visaNumber ILIKE :s OR visa.visaType ILIKE :s)', {
        s: `%${query.search}%`,
      });
    ...
    const enriched = data.map((v) => ({
      ...v,
      computedStatus: this.computeStatus(v),
      daysLeft: Math.ceil(
  ...
  async findOne(id: string): Promise<VisaWithComputedStatus> {
    const visa = await this.visaRepo.findOne({
      where: { id },
    ...
    return {
      ...visa,
      computedStatus: this.computeStatus(visa),
      daysLeft: Math.ceil(
        (new Date(visa.expiryDate).getTime() - Date.now()) /
          (1000 * 60 * 60 * 24),
      ),
    };
  }
  ...
  async update(id: string, dto: UpdateVisaDto): Promise<Visa> {
    ...
    if (
      ...
      throw new BadRequestException('Expiry must be after issue');
  ...
  async findExpiring(days = 90) {
    ...
    const visas = await this.visaRepo
      .createQueryBuilder('visa')
      .leftJoinAndSelect('visa.student', 'student')
      .where('visa.expiryDate <= :threshold', {
        threshold: threshold.toISOString().split('T')[0],
      })
      .andWhere('visa.expiryDate >= :now', {
