...
const mockRepo = (): MockVisaRepository => ({
  create: jest.fn(),
  save: jest.fn(),
  findOne: jest.fn(),
...
describe('VisasService', () => {
  ...
  beforeEach(async () => {
    ...
    repo = module.get<MockVisaRepository>(getRepositoryToken(Visa));
  });
  ...
  it('Expiring en 90 días', async () => {
    const qb = {
      ...
      getMany: jest
        .fn()
        .mockResolvedValue([
          { id: '1', expiryDate: new Date().toISOString().split('T')[0] },
        ]),
    };
  ...
  it('Alerta automática debe marcar alertSent', async () => {
    ...
    const qb = {
      ...
      where: jest.fn().mockReturnThis(),
      andWhere: jest.fn().mockReturnThis(),
      getMany: jest.fn().mockResolvedValue([visa]),
    };
    repo.createQueryBuilder.mockReturnValue(qb);
    repo.save.mockResolvedValue({ ...visa, alertSent: true });
    await service.checkExpiringAndAlert();
