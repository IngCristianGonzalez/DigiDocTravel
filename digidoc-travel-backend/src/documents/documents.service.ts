...
import {
  isAllowedUrl,
  hasValidMagicBytes,
} from '../security/helpers/ssrf.helper.js';
...
export class DocumentsService {
  constructor(
    ...
    private readonly historyRepo: Repository<DocumentHistory>,
  ) {}
  ...
  async create(dto: CreateDocumentDto, userId: string): Promise<Document> {
    ...
    if (
      dto.fileType &&
      !['pdf', 'jpg', 'jpeg', 'png'].includes(dto.fileType.toLowerCase())
    )
    ...
    const doc = this.docRepo.create({
      ...
      fileUrl:
        dto.fileUrl ||
        `https://s3.mock/${Date.now()}-${dto.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`,
    } as any);
  ...
  async upload(
    ...
    if (file.mimetype && !allowedMime.includes(file.mimetype)) {
      logSecurityEvent('UPLOAD_INVALID_MIMETYPE', {
    ...
    if (
      file.buffer &&
      file.mimetype &&
      !hasValidMagicBytes(file.buffer, file.mimetype)
    ) {
    ...
    if (!isAllowedUrl(fileUrl))
      throw new BadRequestException('Generated URL not allowed');
  ...
  async findAll(query: PaginationQuery) {
    ...
    if (query.search)
      qb.andWhere('(doc.name ILIKE :search OR doc.type ILIKE :search)', {
        search: `%${query.search}%`,
      });
    if (query.type) qb.andWhere('doc.type = :type', { type: query.type });
    if (query.studentId)
      qb.andWhere('doc.studentId = :sid', { sid: query.studentId });
    if (query.category)
      qb.andWhere('doc.category = :cat', { cat: query.category });
    if (query.status)
  ...
  async findOne(id: string): Promise<Document> {
    const doc = await this.docRepo.findOne({
      where: { id },
      relations: { student: true, history: true },
    });
