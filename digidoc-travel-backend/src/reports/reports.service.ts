...
export class ReportsService {
  ...
  async studentsReport(query: ReportQuery): Promise<ReportPayload> {
    const qb = this.studentRepo.createQueryBuilder('s');
    if (query.country)
      qb.andWhere('s.countryOrigin = :c', { c: query.country });
    if (query.status)
      qb.andWhere('s.status = :s', { s: query.status === 'true' });
    const data = await qb.getMany();
    return { total: data.length, data, generatedAt: new Date() };
  }

  async documentsReport(query: ReportQuery): Promise<ReportPayload> {
  ...
  async visasReport(query: ReportQuery): Promise<ReportPayload> {
    ...
    if (query.expiring === 'true') {
      qb.where('v.expiryDate <= :t', {
        t: threshold.toISOString().split('T')[0],
  ...
  async exportReport(
    ...
    switch (type) {
      case 'students':
        data = await this.studentsReport(query);
        break;
      case 'documents':
        data = await this.documentsReport(query);
        break;
      case 'visas':
        data = await this.visasReport(query);
        break;
      case 'payments':
        data = await this.paymentsReport(query);
        break;
      default:
