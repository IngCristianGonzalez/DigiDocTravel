# CHECKPOINTS — Evaluación del estado final (agnóstico de stack)
...
- [ ] Los `verification.steps` de `kickstart.json` terminan sin errores ni warnings
      (ej: `dotnet build /warnaserror`, `npm run build` + `npm test`, `pytest`).
...
- [ ] Las features de API usan tests de integración contra la app real
      (ej: `WebApplicationFactory<Program>` en .NET, `supertest` en Node).
